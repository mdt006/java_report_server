...
//	@PostConstruct
		...
		while(true){
			...
			if(!runningFlag){
				if(aipInfoList == null || aipInfoList.size() == 0){
					logger.info("BBIN视讯配置信息为空,请先配置apiinfo，程序稍后将重新获取");
					try {
	...
	public BbinLiveJilvEntityMapper getBbinLiveMapper() {
		return bbinLiveMapper;
	}
	public void setBbinLiveMapper(BbinLiveJilvEntityMapper bbinLiveMapper) {
		this.bbinLiveMapper = bbinLiveMapper;
	}
	public BbinPageRecordMapper getPageRecordMapper() {
		return pageRecordMapper;
	}
	public void setPageRecordMapper(BbinPageRecordMapper pageRecordMapper) {
