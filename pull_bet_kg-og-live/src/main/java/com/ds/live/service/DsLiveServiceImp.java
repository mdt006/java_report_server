...
//	@PostConstruct
		...
		while(true){
			...
			if(!runningFlag){
				if(aipInfoList == null || aipInfoList.size() == 0){
					logger.info("OG视讯配置信息为空,请先配置apiinfo，程序稍后将重新获取");
					try {
	...
	public OGLiveEntityMapper getLiveMapper() {
		return liveMapper;
	}
	public void setLiveMapper(OGLiveEntityMapper liveMapper) {
