		...
		try{
			...
			/*	String param = "website="+LiveConfig.BBIN_LIVE_WEBSITE+"&action=BetTime"+"&uppername="+uppername+
				...
				try{
					...
					String result = WebHTTPUtils.sendPost1(reportUrl+"BetRecord",param);
					...
					if(obj.getBoolean("result")){
						...
						/*	N：没有结果
							...
							*/
							...
							//liveEntity.setOrderDate(java.sql.Date.valueOf(dataJson.getString("Wagers")));
							liveEntity.setOrderDate(Timestamp.valueOf(dataJson.getString("WagersDate")));
							//文档没有这个字段
							...
							/*if(StringUtils.isNotBlank(dataJson.getString("PayoutTime")) &&
									!"null".equals(Timestamp.valueOf("PayoutTime"))){
							...
							if(updateFlag){  //视讯修改的时候不用判断
								...
								//移到这里就行了
								liveEntity.setModifiedDate(new Date());
								liveEntity.setPayoutTime(new Date());
								liveEntity.setAccountDate(new Date());
								setUpdateTime(liveEntity);
